...
Merged_data/
log/
# 纯代码仓常见：忽略实验产物（按需取消注释；需长期保留实验时请用对象存储/独立备份仓，勿误以为可与旧 checkpoints/、log/ 一并丢弃）
# runs/
*.pt
*.pth
*.bin
...
**关于 `runs/`**：新版主线正式训练/评测产物根目录为 **`runs/`**（见根目录 `README.md`）。模板中 `checkpoints/`、`log/` 针对旧布局或本地遗留目录；若你在 `.gitignore` 里忽略 `runs/` 是为了减轻「纯代码仓」体积，需要保留实验时仍应另行备份（对象存储、独立仓库或选择性跟踪），**不要**默认将其与旧目录等同为「可随手删、无备份」。
