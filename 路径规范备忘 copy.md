...
本文档与仓库内 `code/paths_config.py` 及各 `sh/*.sh` 行为一致，并与 **[`sh/用法速查 copy 2.md`](sh/用法速查%20copy%202.md)**、**[`sh/脚本参数说明 copy 2.md`](sh/脚本参数说明%20copy%202.md)** 中「`checkpoints/` 与 `log/` 路径」章节对齐（备忘侧重总览与排障，细节命令仍以该两份为准）。
...
**解析规则**（与 `paths_config.py` 一致；列顺序同 [`脚本参数说明 copy 2.md`](sh/脚本参数说明%20copy%202.md)）：
...
**主日志相对路径**（`run_step3.sh` / `run_step4.sh` / `run_step5.sh` 与 `paths_config` 配合）
...
- Step 3 / Step 5 将上述路径传给 **`AdvTrain.py` / `run-d4c.py` 的 `--log_file`**，由 Python **`FileHandler`** 写入；Step 4 传给 **`generate_counterfactual.py --log_file`**（**`PerfMonitor`** / **`append_log_dual`**）。前台**不对**该 `train.log` 再 **`tee`**。
...
- Step 3 / Step 4 在 **`--all` 且 `--daemon`** 时：终端汇总为 **`step3_daemon_*.log`** / **`step4_daemon_*.log`**；Step 3 **`--eval-only`** 时为 **`step3_eval_daemon_*.log`**。**`run_step5.sh`** 无 **`--all`**，不产生旧版 **`step5_daemon_*.log`**。
...
- 与 **`log/`**（无 s）是不同目录；使用 `sh/run_step3.sh`、`run_step4.sh`、`run_step5.sh` 时应以脚本指定的 **`get_log_task_dir` + `runs/…/train.log`** 为主排查。
...
| `D4C_LOG_STEP` | **仅日志**：非空时为 `log/<task>/<STEP>/`；`run_step4.sh` 默认 `step4`（变量未设置时）；**空字符串**表示改回随 `D4C_CHECKPOINT_*` |
...
*与 [`sh/用法速查 copy 2.md`](sh/用法速查%20copy%202.md)、[`sh/脚本参数说明 copy 2.md`](sh/脚本参数说明%20copy%202.md) 路径章节同步。生成依据：`code/paths_config.py`、`code/train_logging.py`（eval 全局目录），`sh/run_step1_step2.sh`、`run_step3.sh`、`run_step4.sh`、`run_step5.sh`、`run_step5_all.sh`、`run_step3_to_step5_single.sh`、`run_step3_to_step5_all.sh`，`tmux-slurm/submit_d4c.sh`。*
