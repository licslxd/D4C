...
主线请用 ``python code/d4c.py step4|step5 …``（由 runners 内 torchrun 调度）。
"""
import os
import sys
...
from paths_config import get_data_dir, get_merged_data_dir, get_t5_small_dir
import torch
...
if __name__ == "__main__":
    ...
    encoded_data = datasets.map(lambda sample: processor(sample), num_proc=4, desc="Tokenize")
    ...
    suser_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.auxiliary, "user_profiles.npy")), dtype=torch.float, device=config.get("device"))
    sitem_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.auxiliary, "item_profiles.npy")), dtype=torch.float, device=config.get("device"))
    sdomain_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.auxiliary, "domain.npy")), dtype=torch.float, device=config.get("device"))
    tuser_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.target, "user_profiles.npy")), dtype=torch.float, device=config.get("device"))
    titem_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.target, "item_profiles.npy")), dtype=torch.float, device=config.get("device"))
    tdomain_profiles = torch.tensor(np.load(os.path.join(get_data_dir(), args.target, "domain.npy")), dtype=torch.float, device=config.get("device"))
