...
_STEP4_REQUIRES_TORCHRUN_MSG = (
    "generate_counterfactual.py（Step 4）仅支持在 torchrun / python -m torch.distributed.run 下以 DDP 运行"
    "（含 --nproc_per_node=1 单卡 DDP smoke）。\n"
    "请勿使用 `python generate_counterfactual.py` 直接启动。\n"
    "示例（在 code/ 目录）：\n"
    "  DDP_NPROC=1 torchrun --standalone --nproc_per_node=1 generate_counterfactual.py "
    "[--task N] [--batch-size G] [--log_file PATH]\n"
...
if __name__ == "__main__":
    for _arg in sys.argv[1:]:
        if _arg == "--gpus" or _arg.startswith("--gpus="):
            sys.stderr.write(
                "generate_counterfactual.py: error: --gpus has been removed.\n"
                "Use CUDA_VISIBLE_DEVICES and torchrun --standalone --nproc_per_node=<K> generate_counterfactual.py ...\n"
                "Single-GPU DDP smoke: torchrun --standalone --nproc_per_node=1 generate_counterfactual.py ...\n"
            )
            raise SystemExit(2)

    parser = argparse.ArgumentParser(
        description="Step 4 反事实生成：须 torchrun DDP（含 nproc_per_node=1）",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="PerfMonitor 结构化日志路径；不传则沿用当前目录 save.log（与 run_step4_optimized.sh 传入的绝对路径对齐 Step 3/5）",
    )
