        ...
        for epoch in range(epochs):
            ...
            if rank == 0:
                ...
                if checkpoint_metric == "bleu4" and valid_dataset_for_bleu is not None:
                    bleu4_score_this_epoch = _valid_bleu4_quick_d4c(
    ...
    try:
        ...
        if run_final_eval:
            ...
            eval_dataloader = DataLoader(
                ...
                shuffle=False,
                num_workers=max(1, min(max(1, get_dataloader_num_workers("train") // max(world_size, 1)), get_max_parallel_cpu()) // 2),
                pin_memory=torch.cuda.is_available(),
            )
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="BLEU-4 选模时验证集最多条数；默认 TRAIN_BLEU4_MAX_SAMPLES",
    )
